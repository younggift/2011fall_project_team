 ...
 new Thread(new Runnable() {
                   ...
                   else  if(command1==3)
           {
               ...
               int number=(int)(Math.random()*Dizhu.size())+1;
                if(Dizhu.isEmpty())
                {
                    JOptionPane.showMessageDialog(null,"没人抢地主");
                    System.exit(-1);
                }
...
{ 
    ThreadServer ts=threadArray.get(i);
if(ts.name.equals("1")) ts.send("11|"+1+test.user1); 
else if(ts.name.equals("2")) ts.send("11|"+2+test.user2); 
else if(ts.name.equals("3")) {
    ts.send("11|"+3+test.user3); 
}
  ...
  public void  AnMsg(String msg)  //解析消息
    {
     ...
     int i=0;
       while (st.hasMoreTokens())
       {
           ...
           else if(i==1) Recname=st.nextToken();
       else   
               {
